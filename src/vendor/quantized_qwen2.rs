...
// Vendored from candle-transformers 0.11.0 (src/models/quantized_qwen2.rs), MIT/Apache-2.0.
// Local change: the token embedding table is stored as f16 instead of
// f32 (~260MB RSS saved for Qwen2.5-0.5B); the lookup output is cast back
// to f32 so RMSNorm/attention activations keep full precision.
use candle_core::quantized::QMatMul;
use candle_core::{quantized::gguf_file, DType, Device, IndexOp, Result, Tensor};
...
impl ModelWeights {
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let tok_embeddings = tok_embeddings.dequantize_f16(device)?;
