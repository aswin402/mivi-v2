//! Native CPU inference engine: candle-backed GGUF generation, sampling,
//! and streaming.
//!
//! Extracted from `native_brain.rs` (decomposition); model loading and the
//! model cache live in `native_model.rs`.
...
use crate::native_model::{
    common_prefix_len, find_tokenizer_path, load_grammar_state, model_cache_max_entries,
    LoadedModel, ModelCache, QuantizedModel,
};
#[cfg(feature = "native")]
use std::fs::File;
...
impl NativeBrain {
    ...
    pub fn query(
        ...
        let formatted_prompt = if crate::reasoning::is_prompt_preformatted(prompt) {
            prompt.to_string()
        } else {
        ...
        *loaded.cached_prefix.lock().unwrap() = token_ids.clone();
        ...
        Ok(crate::reasoning::strip_think_blocks(&cleaned))
    }
