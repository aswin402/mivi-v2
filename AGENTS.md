...
- `src/brain.rs` — `EdgeBrain`: subprocess wrapper around `llama-cli` / `llama-mtmd-cli`; prompt/system-user splitting; model-path and CLI config resolution. Reasoning directives and think-block/response cleaning live in `src/reasoning.rs`.
- `src/reasoning.rs` — Qwen3 reasoning-mode directives (`/think`, `/no_think`, `MIVI_REASONING_MODE`), `strip_think_blocks`, and llama-cli response scrubbing.
- `src/native_brain.rs` — candle-backed native CPU inference (`query`, `query_raw_prompt`, `query_stream`); model loading, tokenizer discovery, grammar state, and the LRU `ModelCache` live in `src/native_model.rs`.
